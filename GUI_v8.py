...
class Curate_tracks(MagicTemplate):
    ...
    def __init__(self, target: ImageData, 
         ...
         self.phase   = phase  # Phase image for convolution 
    ...
    def _displayTrack(self):
        ...
        # Calculate shapescore from the phase image.
        phase_roi = utils.getROI(self.phase, currentTrack, self.pars.roisize)
        
        kernel = utils.drawAnnulus(81,32,35,(40,40))
        centroid, shapescore = cycb.shapeMetric(phase_roi, 
                                      kernel, 
                                      self.pars.binsize)
        mitosisflag = 220*np.ones_like(shapescore)
        mitosisflag[shapescore>0.13] = 250
    ...
    def _saveTrack(self):
        selectedTrack = int(self.Frame1.TrackNum.value)
        currentID    = self.IDs[self.IDs[selectedTrack]]
        self.cycb_data[currentID] = self.sig_arr
